...
#   142.X11 Access (Checks if you're allowed to connect to the X server)"""
    ...
    if ans=="15":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p15 = subprocess.Popen(['nmap', '--script=auth', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Auth Scan please be patient...") % remoteServer
      output15 = p15.communicate()[0]
      print output15 
     if xmloutput == "no":
      p15 = subprocess.Popen(['nmap', '--script=auth', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Auth Scan please be patient...") % remoteServer
      output15 = p15.communicate()[0]
      print output15
    ...
    if ans=="16":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p16 = subprocess.Popen(['nmap', '--script=broadcast', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Broadcast Scan please be patient...") % remoteServer
      output16 = p16.communicate()[0]
      print output16 
     if xmloutput == "no":
      p16 = subprocess.Popen(['nmap', '--script=broadcast', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Broadcast Scan please be patient...") % remoteServer
      output16 = p16.communicate()[0]
      print output16
    ...
    if ans=="17":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p17 = subprocess.Popen(['nmap', '--script=brute', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Brute Scan please be patient...") % remoteServer
      output17 = p17.communicate()[0]
      print output17 
     if xmloutput == "no":
      p17 = subprocess.Popen(['nmap', '--script=brute', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Brute Scan please be patient...") % remoteServer
      output17 = p17.communicate()[0]
      print output17  
    ...
    if ans=="18":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p18 = subprocess.Popen(['nmap', '--script=default', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Default Scan please be patient...") % remoteServer
      output18 = p18.communicate()[0]
      print output18 
     if xmloutput == "no":
      p18 = subprocess.Popen(['nmap', '--script=default', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Default Scan please be patient...") % remoteServer
      output18 = p18.communicate()[0]
      print output18 
    ...
    if ans=="19":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p19 = subprocess.Popen(['nmap', '--script=discovery', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Discovery Scan please be patient...") % remoteServer
      output19 = p19.communicate()[0]
      print output19 
     if xmloutput == "no":
      p19 = subprocess.Popen(['nmap', '--script=discovery', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Discovery Scan please be patient...") % remoteServer
      output19 = p19.communicate()[0]
      print output19 
    ...
    if ans=="20":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p20 = subprocess.Popen(['nmap', '--script=dos', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Dos Scan please be patient...") % remoteServer
      output20 = p20.communicate()[0]
      print output20 
     if xmloutput == "no":
      p20 = subprocess.Popen(['nmap', '--script=dos', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Dos Scan please be patient...") % remoteServer
      output20 = p20.communicate()[0]
      print output20 
    ...
    if ans=="21":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p21 = subprocess.Popen(['nmap', '--script=exploit', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Exploit Scan please be patient...") % remoteServer
      output21 = p21.communicate()[0]
      print output21 
     if xmloutput == "no":
      p21 = subprocess.Popen(['nmap', '--script=exploit', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Exploit Scan please be patient...") % remoteServer
      output21 = p21.communicate()[0]
      print output21
    ...
    if ans=="22":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p22 = subprocess.Popen(['nmap', '--script=external', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with External Scan please be patient...") % remoteServer
      output22 = p22.communicate()[0]
      print output22 
     if xmloutput == "no":
      p22 = subprocess.Popen(['nmap', '--script=external', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with External Scan please be patient...") % remoteServer
      output22 = p22.communicate()[0]
      print output22
    ...
    if ans=="23":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p23 = subprocess.Popen(['nmap', '--script=fuzzer', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Fuzzer Scan please be patient...") % remoteServer
      output23 = p23.communicate()[0]
      print output23 
     if xmloutput == "no":
      p23 = subprocess.Popen(['nmap', '--script=fuzzer', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Fuzzer Scan please be patient...") % remoteServer
      output23 = p23.communicate()[0]
      print output23
    ...
    if ans=="24":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p24 = subprocess.Popen(['nmap', '--script=intrusive', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Intrusive Scan please be patient...") % remoteServer
      output24 = p24.communicate()[0]
      print output24 
     if xmloutput == "no":
      p24 = subprocess.Popen(['nmap', '--script=intrusive', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Intrusive Scan please be patient...") % remoteServer
      output24 = p24.communicate()[0]
      print output24
    ...
    if ans=="25":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p25 = subprocess.Popen(['nmap', '--script=malware', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Malware Scan please be patient...") % remoteServer
      output25 = p25.communicate()[0]
      print output25 
     if xmloutput == "no":
      p25 = subprocess.Popen(['nmap', '--script=malware', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Malware Scan please be patient...") % remoteServer
      output25 = p25.communicate()[0]
      print output25
    ...
    if ans=="26":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p26 = subprocess.Popen(['nmap', '--script=safe', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Safe Scan please be patient...") % remoteServer
      output26 = p26.communicate()[0]
      print output26 
     if xmloutput == "no":
      p26 = subprocess.Popen(['nmap', '--script=safe', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Safe Scan please be patient...") % remoteServer
      output26 = p26.communicate()[0]
      print output26
    ...
    if ans=="27":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p27 = subprocess.Popen(['nmap', '--script=version', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Version Scan please be patient...") % remoteServer
      output27 = p27.communicate()[0]
      print output27 
     if xmloutput == "no":
      p27 = subprocess.Popen(['nmap', '--script=version', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Version Scan please be patient...") % remoteServer
      output27 = p27.communicate()[0]
      print output27
    ...
    if ans=="28":
     xmloutput = raw_input("Save scan in XML format? [yes|no]:")
     if xmloutput == "yes":
      p28 = subprocess.Popen(['nmap', '--script=vuln', '-v', '-oX', 'nmap_scan-%T-%D.xml', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Vuln Scan please be patient...") % remoteServer
      output28 = p28.communicate()[0]
      print output28 
     if xmloutput == "no":
      p28 = subprocess.Popen(['nmap', '--script=vuln', '-v', remoteServer], stdout=sys.stdout, stderr=sys.stderr)
      print colorgrn.format("Scanning %s with Vuln Scan please be patient...") % remoteServer
      output28 = p28.communicate()[0]
      print output28
